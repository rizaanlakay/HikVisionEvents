...
{
    public partial class HikVisionService : ServiceBase
    {
        ...
        public HikVisionService()
        {
            ...
            var sSource = "HikVision Event Service";

            if (!EventLog.Exists(sLog) || EventLog.LogNameFromSourceName(sSource, ".") != sLog)
            {
            ...
            eventLog1.Log = sLog;
            eventLog1.WriteEntry("HikVision Event Service initializing...");
        }
        ...
        private void InitializeNVR()
        {
            ...
            if (m_bInitSDK == false)
            {
                eventLog1.WriteEntry("Error Initializing the NVR.", EventLogEntryType.Error);
            }
            else
            {
                ...
                eventLog1.WriteEntry("NVR Successfully connected.", EventLogEntryType.Information);
            }
        }
        #endregion

        #region System OnStart and OnStop Events
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                eventLog1.WriteEntry("MQTT Server configured.");
        ...
        private bool LoginToNVR()
        {
            if (m_lUserID < 0)
            {
                string DVRIPAddress = ConfigurationManager.AppSettings["NVRIP"];
                Int16 DVRPortNumber = short.Parse(ConfigurationManager.AppSettings["NVRPort"]);
                string DVRUserName = ConfigurationManager.AppSettings["NVRUsername"];
                string DVRPassword = ConfigurationManager.AppSettings["NVRPassword"];
                ...
                else
                {
                    ...
                    eventLog1.WriteEntry("NVR Successfully connected.");
                    return true;
                }
        ...
        private void ArmAllDevices()
        {
            ...
            if (m_lAlarmHandle[m_lUserID] < 0)
            {
                ...
                eventLog1.WriteEntry("Error: " + iLastErr, EventLogEntryType.Error);
            }

            eventLog1.WriteEntry("All NVR devices armed.");

        }
        private void DisarmDevices()
        {
            if (m_lAlarmHandle[m_lUserID] >= 0)
            {
                ...
                {
                    iLastErr = CHCNetSDK.NET_DVR_GetLastError();
                    string strErr = "Failed to disarm, Error code:" + iLastErr;  // Write disarm failed log
                }
                else
                {
        ...
        private async void ProcessMQTTMessage(DateTime eventTime, AlarmEventTypes alarmType, string ipAddress, string alarmMessage)
        {
            // Send MQTT message to Home Assistant.
            alarmMessage = alarmMessage.Replace(@"\","").Trim();
            ...
            var _payload = "{\"datetime\":\"" + eventTime.ToString() + "\", \"alarmtype\":\"" + alarmType + "\", \"ip\":\"" + ipAddress + "\", \"message\":\"" + alarmMessage + "\"}";
            ...
            eventLog1.WriteEntry("NVR Event occurred. Payload: " + _payload);
